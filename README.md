...
2. Resize all images to a fixed size (128×128)
3. Normalize pixel values to the range [0, 1]
4. Split data into training and validation sets (80/20)
5. Apply data augmentation to the training set only
...
2. Upload the Weather dataset (image folders) to the Colab session.
3. Install the required libraries (if they are not already available).
4. Run all the notebook cells sequentially.
